...
function updateHouseData(action) {
    ...
    oldSd = location.search.split('&')[2].split('=')[1];
    oldEd = location.search.split('&')[3].split('=')[1];
    path = path.replace('sd=' + oldSd, 'sd=' + startDate);
    path = path.replace('ed=' + oldEd, 'ed=' + endDate);
    location.href = path
...
$(document).ready(function(){
    ...
    var sortKey = queryData["sk"];
    if(sortKey == 'booking'){
        var sortName = '入住最多'
    }else if (sortKey == 'price-inc'){
        var sortName = '价格 低-高'
    }else if (sortKey == 'price-des'){
        var sortName = '价格 高-低'
    }else {
        var sortName = '最新上线'
    }
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        ...
        location.href = location.search.split('&sk=')[0] + '&sk=' + sortKey
...
$(function () {
            ...
            $.get('/searchhouse/' + path, function (data) {

                if(data.code == 200){

                    var house_info_search = template('house_info_search', {hlist:data.hlist})
                    $('.house-list').html(house_info_search);
